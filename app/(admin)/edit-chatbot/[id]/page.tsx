...
function EditChatbot() {
  ...
  useEffect(() => {
    if (data) {
      console.log(data);
      setChatbotName(data.data.chatbots.name);
    }
  }, [data]);
  ...
  const handelUpdateChatbot = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.promise(promise, {
        loading: "Deleting...",
        success: "Chatbot Successfully deleting!",
  ...
  if (!data?.data?.chatbots) return redirect("/view-chatbots");
      ...
      <section className="relative mt-5 bg-white p-5 md:p-10 rounded-lg">
        ...
        <div className="bg-gray-200 p-5 md:p-5 rounded-md mt-5">
          ...
          <ul className="flex flex-wrap-reverse gap-5 ">
            {data?.data?.chatbots?.chatbot_characteristics?.map(
              characteristic => (
                <Characteristic
